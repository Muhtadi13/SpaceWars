...
/*shows list of 5 players who made highscore*/
void hall_of_fame()
{
    int no_of_data;
    ...
    printf("no of data in prev record=%d\n",no_of_data);
    no_of_data++;
    ...
    for(int it=0; it<no_of_data-1; it++)
    {
        ...
        fscanf(database,"%d %s",&player_data[it].score,player_data[it].name);
    ...
    else
    {
        ...
        player_data[no_of_data-1].score=0;
        printf("no of data in debugged record=%d\n",no_of_data);
    }
    ...
    printf("just checking number of data: %d\n",no_of_data);
    for(int it=0; it<no_of_data; it++)
    {
        if(point>=player_data[it].score)
        {

            printf("eikhane dhuksi\n");
            ...
            trimLeading(player_data[it].name);
            printf("checking insertion(from):%d %s\n",point,nameofplayer);
            printf("checking insertion(to):%d %s\n",player_data[it].score,player_data[it].name);
    ...
    for(int it=0; it<no_of_data; it++)
    {
        ...
        if((player_data[it].score>0 && player_data[it].score<=100000)&&((player_data[it].name[0]>='A' && player_data[it].name[0]<='Z')||(player_data[it].name[0]>='a' && player_data[it].name[0]<='z') || (player_data[it].name[0]>='0' && player_data[it].name[0]<='9') ))
        {
            ...
            fprintf(newdata,"%d %s\n",player_data[it].score,player_data[it].name);
            printf("written%d: %d %s\n",it,player_data[it].score,player_data[it].name);
        }
        else
        {
            break;
...
/*initiallizes m th bullet of enemy*/
...
/*function to demonstrate collision between planes and state of being shot*/
void collision(void)
{
...
/*drawings of different states of game*/
void iDraw()
{
    iClear();
    ...
    else if(gamestate==0)
    {
        ...
        if(resetstate==1)                /*resets the game*/
        {
    ...
    else if(gamestate==1)
    {
        ...
        iSetColor(255,155,0);
        iText(l/3,w-500,"Use mouse to click menu",GLUT_BITMAP_9_BY_15);
    ...
    if(gamestate==4)
    {
        ...
        if(num<5)
        {
            for(int z=0; z<num; z++)
            {
                ...
                iText(l/3,ranky,rankname,GLUT_BITMAP_TIMES_ROMAN_24);
                iText(1160,ranky,rankscore,GLUT_BITMAP_TIMES_ROMAN_24);
...
void iMouse(int button,int state,int mx,int my)
{
    if(button==GLUT_LEFT_BUTTON && state==GLUT_DOWN)
    {
        ...
        if((gameover==1 || gamestate!=0) && mx>=savebutton.x && mx<=savebutton.x+46 && my>=savebutton.y && my<=savebutton.y+49 && saved==0)
        {
            saved=1;
            if(musicon==true)
                PlaySound("select.wav", NULL,SND_ASYNC);
        ...
        if(gamestate==-1 && mx>=rankbutton.x && mx<=rankbutton.x+32 && my>=rankbutton.y && my<=rankbutton.y+32)
        {
            if(musicon==true)
                PlaySound("select.wav", NULL,SND_ASYNC);
            showrank=1;
