...
int planewidth=150, myplaneht=35;
int eplanewidth=110, eplaneht=30;
int bottomht=100;
...
void hall_of_fame(){
    ...
    if(fp==NULL){
        printf("error reading name file\n");
            ...
            if(nameind==0){strcpy(nameofplayer,"UNKNOWN PLAYER");}
          else if(nameind>0){
            do{
...
if(resumestate==0 && gameover==0)
    {
        ...
        if(gameover==1)
        {
            ...
            iText(l/2-350,w/2-100,"YOUR SCORE IS",GLUT_BITMAP_TIMES_ROMAN_24);
            iText(l/2+50,w/2-100,score_point,GLUT_BITMAP_TIMES_ROMAN_24);
            iText(l/2-350,w/2-200,"HIGHEST SCORE:",GLUT_BITMAP_TIMES_ROMAN_24);
            iText(l/2+50,w/2-200,highscore,GLUT_BITMAP_TIMES_ROMAN_24);
...
void iMouse(int button,int state,int mx,int my)
{
    ...
    if(button==GLUT_RIGHT_BUTTON && state==GLUT_DOWN)
    {
        ...
        if(gameover==1 && mx>=l-200 && mx<=(l-150) && my>=(bottomht-20) && my<=(bottomht+30) && saved==0){
            saved=1;
             if(musicon==true)
                        PlaySound("select.wav", NULL,SND_ASYNC);
                        savebutton.x=l+200;
