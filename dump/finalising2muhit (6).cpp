...
//            printf("checking:%c\t%d",playername[nameind],write);
    ...
    if(gamestate==4){
        ...
        iText(l/2,w-300,"RANKS: ",GLUT_BITMAP_TIMES_ROMAN_24);

        iText(l/3,w-400,"NAME",GLUT_BITMAP_TIMES_ROMAN_24);
        iText(l/2+100,w-400,"SCORE",GLUT_BITMAP_TIMES_ROMAN_24);
...
void iMouse(int button,int state,int mx,int my)
{
    if(button==GLUT_LEFT_BUTTON && state==GLUT_DOWN)
    {
        ...
        if(mx>=rankbutton.x && mx<=rankbutton.x+50 && my>=rankbutton.y && my<=rankbutton.y+50){
             if(musicon==true)
                PlaySound("select.wav", NULL,SND_ASYNC);
            showrank=1;
            gamestate=4;
