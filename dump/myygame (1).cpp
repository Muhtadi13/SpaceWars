...
int myx=100, myy=300;
int l=1200, w=700;
int my_missile_x[100000], my_missile_y[100000], n=0, m1=3;
int i, j;
...
bool musicon=true;
char bg[6][20]={"front.bmp","gamestartbg.bmp","gamebg6.bmp","skyfighter2.bmp","space.bmp"};
int bgind=0;
...
void enemy_movement(void)
{
    ...
    for (i=0; i<enemynumber; i++)
    {
        ...
        {
            e[i].ex=l+10;
            e[i].ey=rand()%(w-eplaneht)+bottomht;
                for (j=0; j<i; j++){
...
void collision(void)
{
   ...
   /*if(e[ind].enemyshot==1)
   {
    ...
    for(int ind=0; ind<enemynumber; ind++)
    {
        for( i=0; i<n; i++)
        {
            if(my_missile_x[i]>e[ind].ex+5 && my_missile_x[i]<(e[ind].ex+eplanewidth) && my_missile_y[i]>(e[ind].ey-5) && my_missile_y[i]<(e[ind].ey+eplaneht))
            {
              e[ind].enemyshot=1;
/*
    ...
    else if(gamestate==0)
    {
       ...
       iShowBMP(0,bottomht,bg[4]);
        ...
        for (i=0; i<enemynumber; i++)
        {
            ...
            if(e[i].enemyshot==1 && e[i].enemytime<=50 ){
