...
void hall_of_fame()
{
    ...
    else
    {
        ...
        printf("\n no of records didnt match at first\n\n");
    }
 ...
 //   int it=0,jt=0;
    ...
    for(int it=0; it<no_of_data; it++)
    {
        ...
        fprintf(newdata,"%d %s",player_data[it].score,player_data[it].name);
        //fputs(player_data[it].name,newdata);
...
int newenemyY(){
    ...
    int no;
    int eyy=rand%(w-bottomht-eplaneht-10)+bottomht;

    for(no=0;no<enemynumber,no++){
        if(e[])
    }
...
//            printf("checking:%c\t%d",playername[nameind],write);
    ...
    {
        iSetColor(rand()%255,rand()%255,rand()%255);
        //iText(l/3,300+50,"Settings menu under development");
        ...
        iSetColor(0,255,255);
        iText(l/3,w-500,"Use mouse to click menu",GLUT_BITMAP_TIMES_ROMAN_10);
        //iShowBMP(l-150,bottomht-20,"exitgame.bmp");
        doexit.x=l-150;
        doexit.y=bottomht-50;
    ...
    if(gamestate==4){
        ...
        iText(l/2-100,w-100,"LEADERBOARD: ",GLUT_BITMAP_TIMES_ROMAN_24);

        iText(l/3,w-200,"NAME",GLUT_BITMAP_TIMES_ROMAN_24);
        iText(l/2+100,w-200,"SCORE",GLUT_BITMAP_TIMES_ROMAN_24);
           ...
           //  if(showrank==1){
            if(num<5){
                    for(int z=0;z<num;z++){
                        ...
                        printf("rankname=%s\n",rankname);
                        iText(l/3,ranky,rankname,GLUT_BITMAP_HELVETICA_18);
                        iText(l/2+100,ranky,rankscore,GLUT_BITMAP_HELVETICA_18);
...
void iMouse(int button,int state,int mx,int my)
{
    if(button==GLUT_LEFT_BUTTON && state==GLUT_DOWN)
    {
        ...
        if(gamestate==-1 && mx>=rankbutton.x && mx<=rankbutton.x+50 && my>=rankbutton.y && my<=rankbutton.y+50){
             if(musicon==true)
                PlaySound("select.wav", NULL,SND_ASYNC);
            showrank=1;
            gamestate=4;
